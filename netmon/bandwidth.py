...
def sample(interval_seconds: float = 5.0) -> tuple[float, float]:
    ...
    before = psutil.net_io_counters(pernic=True)[iface]
    time.sleep(interval_seconds)
    after = psutil.net_io_counters(pernic=True)[iface]

    dl = _bytes_to_mbps(after.bytes_recv - before.bytes_recv, interval_seconds)
    ul = _bytes_to_mbps(after.bytes_sent - before.bytes_sent, interval_seconds)
    return dl, ul
